...
<div
    ...
    <div class="flex shadow-md my-2 bg-white rounded p-2 items-center mb-5">
    ...
    <form method="post" action="{{url('shorten')}}">
        ...
        <div class="flex">
            ...
            <button type="submit" class="bg-orange-500 hover:bg-orange-400 text-white font-bold py-2 px-4 border-b-4 border-orange-700 hover:border-orange-500 rounded ">
                Shorten
            </button>
        </div>
    ...
    <div class="flex flex-col">
        @foreach($myUrls as $url)
            <div class="p-4 shadow-md my-2 justify-center flex flex-col bg-white rounded">
                <p><span class="font-bold">Short: </span> {{$host.'/'.$url->short}}</p>
                <p><span class="font-bold">Destination: </span> {{$url->destination}}</p>
                <a href="/{{$url->short}}?admin"
